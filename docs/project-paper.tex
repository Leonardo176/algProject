...
\begin{document}
    ...
    \subsection{Insertions Tests}
        ...
        \begin{itemize}
            ...
            \item AVL trees enforces the strictest balancing rules to ensure the tree remaines perfectly shallow. However, this come at a steep cost during insertion, as maintaining this strictness could have require up to $O(\log n)$ structural rotations in the worst case. This heavy overhead explained why it had the highest multiplicative constant and the slowest insertion time.
    ...
    \subsection{Rotations Count Tests}
            ...
            Other than that, we observed that the number of rotations on both trees followed a logarithmic trend. To verify this observation, we used \texttt{numpy} to perform a logarithmic regression to approximate the above graph with a logarithmic function. After computing the resulting function revealed as 
    ...
    \subsection{Heights Tests}
        ...
        These measurements gave us a practical understanding on how effectively self-balancing tecniques are able to control height during tree updates. As a direct consequence, the computational cost for any search operation results significantly optimized in AVL and RBT structures, compared to standard BST.
